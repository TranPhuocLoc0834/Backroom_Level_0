...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        {
            // xoay camera dựa trên input
            if (_input.look.sqrMagnitude >= _threshold && !LockCameraPosition)
            {
                float deltaTimeMultiplier = IsCurrentDeviceMouse ? 1.0f : Time.deltaTime;
                _cinemachineTargetYaw += _input.look.x * deltaTimeMultiplier;
                _cinemachineTargetPitch += _input.look.y * deltaTimeMultiplier; // FPS đảo trục Y
            }

            _cinemachineTargetPitch = Mathf.Clamp(_cinemachineTargetPitch, BottomClamp, TopClamp);

            // Nhân vật xoay theo yaw, camera xoay pitch
        ...
        private void Move()
        {
            ...
            Vector3 moveDirection = Quaternion.Euler(0f, _cinemachineTargetYaw, 0f) * inputDirection;
